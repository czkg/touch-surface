...
bool BoundingBox::create_box_OBB(float* xyz_data, const int& data_num)
{
	m_xyz_data.clear();
	...
	{
		if (fabs(xyz_data[3 * i + 2]) < FLT_EPSILON)	// depth == 0.0 then skip
		{
			continue;
		}
		m_xyz_data.push_back(Float4(xyz_data[3 * i],
		xyz_data[3 * i + 1], xyz_data[3 * i + 2], 1.0f));

		for (int j = 0; j < 3; ++j)
		{
			center_pt[j] += xyz_data[3 * i + j];			// 3 * i + j
		}		
	}
...
bool BoundingBox::project_direct(cv::Mat* proj_im, const int& sz)
...
// 0: x-y, 1: y-z, 2: z-x
{
	...
	{
		proj_bounding_box[0].height = sz - (pad * 2.0);
	...
	proj_bounding_box[0].x = ((sz - proj_bounding_box[0].width) / 2.0);
	...
	{
		proj_bounding_box[1].height = sz - (pad * 2.0);
	...
	proj_bounding_box[1].x = ((sz - proj_bounding_box[1].width) / 2.0);
	...
	{
		proj_bounding_box[2].height = sz - (pad * 2.0);
	...
	proj_bounding_box[2].x = ((sz - proj_bounding_box[2].width) / 2.0);
...
math::Float3 BoundingBox::get_yaw_pitch_roll()
{
